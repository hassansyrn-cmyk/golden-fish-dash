...
export interface PowerUp {
  x: number;
  y: number;
  type: 'shield' | 'magnet' | 'fever' | 'slowmo';
  collected: boolean;
  pulse: number;
}
...
export interface EngineState {
  ...
  // Hourglass drop: temporarily slows the world down
  slowMoUntil: number;
}
...
export function difficultyForScore(score: number, timeMs: number = 0) {
  // Speed ramps up gently as the score increases, then locks in place for good.
  // There is NO time-based acceleration anymore — surviving a long run no longer
  // keeps making the game faster forever. Once speedSteps caps out (score 72),
  // speed is completely flat for the rest of the run, so the game stays winnable.
  const speedSteps = Math.min(Math.floor(score / 12), 6);
  const speed = Math.min(BASE.maxSpeed * 0.65, BASE.baseSpeed + speedSteps * 0.22);

  // Gap and spawn rate follow the same gentle, capped ramp (no run-time penalty).
  const gap = Math.max(BASE.minGap + 24, BASE.baseGap - speedSteps * 4);
  const spawnInterval = Math.max(1150, BASE.spawnInterval - speedSteps * 38);

  // Enemy/hazard spawn-chance scaling still eases in with score for variety,
  // but is now capped modestly instead of compounding without limit.
  const diffMultiplier = Math.min(1 + score / 300, 1.5);
...
function spawnObstacle(state: EngineState, score: number) {
  ...
  // Power-up spawn (shield, magnet, Fever mode Star, or Hourglass slow-mo)
  if (Math.random() < 0.09) {
    const roll = Math.random();
    const type: 'shield' | 'magnet' | 'fever' | 'slowmo' =
      roll < 0.3 ? 'shield' : roll < 0.55 ? 'magnet' : roll < 0.75 ? 'fever' : 'slowmo';
...
export function stepEngine(state: EngineState, dtMs: number, callbacks: EngineCallbacks, settings: { vibration: boolean }) {
  ...
  let { speed, spawnInterval } = difficultyForScore(state.score, state.timeMs);
  const isSlowMoActive = state.slowMoUntil > state.timeMs;
  if (isSlowMoActive) {
    speed *= 0.5; // Hourglass pickup: halve world speed for a few seconds
    spawnInterval /= 0.5;
  }
    ...
    if (!pu.collected) {
      ...
      if (Math.sqrt(dx * dx + dy * dy) < BASE.fishRadius + 18) {
        ...
        if (pu.type === 'shield') {
          ...
          state.slowMoUntil = state.timeMs + 5000;
          triggerFloatingText(state, '⏳ Slow-Mo!', pu.x, pu.y - 15, '#80deea', true);
          addBurst(state, pu.x, pu.y, 'rgba(128, 222, 234, 0.95)', 20, 3);
        }
      }
    }
  }
