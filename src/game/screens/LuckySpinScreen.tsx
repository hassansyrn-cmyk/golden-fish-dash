  ...
  const spin = () => {
    ...
    const animate = () => {
      ...
      if (velocityRef.current < 0.0018) {
        ...
        // The pointer is fixed at the TOP of the wheel (screen angle 1.5*PI in
        // canvas convention). Slices are drawn starting at angle 0 = 3 o'clock
        // and rotate with the wheel, so we need to find which slice's angular
        // range now lines up with 1.5*PI after the wheel's rotation.
        // (Previous code used 2.5*PI, which mod 2*PI collapses to 0.5*PI —
        // the BOTTOM of the wheel — causing the reported prize to be off by
        // half the wheel from what's visually under the pointer.)
        const twoPi = Math.PI * 2;
        const normalizedAngle = (((Math.PI * 1.5) - (currentAngleRef.current % twoPi)) % twoPi + twoPi) % twoPi;
        const idx = Math.floor(normalizedAngle / arcSize) % PRIZES.length;
